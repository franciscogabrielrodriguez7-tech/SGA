...
--   fn_verificar_usuario_activo               -> trg_validar_creador_activo (BEFORE INSERT alquiler)
--
...
--
-- NOTA: el contenido SQL de este archivo era idéntico al original
-- al momento de la reorganización. Posteriormente se agregó la
-- función/trigger fn_verificar_usuario_activo / trg_validar_creador_activo
-- (RN-USR-05: un usuario desactivado no puede operar), a solicitud
-- explícita del usuario, con una corrección: la versión propuesta
-- originalmente consultaba una columna "estado" (VARCHAR) que no
-- existe — la columna real es "estado_usuario" (BOOLEAN). Ver el
-- comentario junto a esa función más abajo para el detalle completo.
-- =========================================================
...
-- =========================================================
-- VALIDAR QUE EL USUARIO CREADOR ESTÉ ACTIVO
-- -----------------------------------------------------------
-- CORREGIDO respecto a la versión propuesta originalmente: esa
-- versión consultaba una columna "estado" (VARCHAR) comparada
-- contra el string 'activo', que no existe en el esquema real.
-- La columna real es usuario.estado_usuario, de tipo BOOLEAN
-- (TRUE = activo, FALSE = inactivo). Usar la columna incorrecta
-- producía el error "no existe la columna «estado»" al intentar
-- crear cualquier alquiler (BEFORE INSERT ON alquiler se ejecuta
-- siempre), lo cual bloqueaba por completo la creación de
-- alquileres desde el frontend.
-- =========================================================
...
BEGIN
    ...
    IF v_estado_usuario IS NULL OR v_estado_usuario IS NOT TRUE THEN
