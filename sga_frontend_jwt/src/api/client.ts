...
//  - adjunta automáticamente el header "Authorization: Bearer <token>" en
//    cada petición, leyendo el token guardado por AuthContext en
//    localStorage
//  - si el backend responde 401 (token vencido/inválido), dispara el
//    evento global "sesionExpirada" para que AuthContext cierre la
...
export async function apiRequest<T>(
  ...
  if (!sobre || !sobre.status) {
    // Token vencido/inválido -> forzar cierre de sesión global. No se
    // hace para la propia ruta de login (401 = credenciales incorrectas,
    // no sesión expirada) para no entrar en bucle.
